...
{
    [TestClass]
    public class When_player1_uses_playstation_controller : with_valid_keys
    {
        private Game _game;

        public override void When()
        {
            _game = new Game("Specs.When_player1_uses_playstation_controller", 1, 1);
            _game.SetWindow(1, 1);
        }
        ...
        public void Then_Dpad_Can_Be_Set()
        {
            using(_game)
            {
                _game.AddSession("Player1", new ControllerPS3());
                var controller = _game.Session("Player1").Controller;

                controller.Enabled = true;
                controller.Enabled.Should().BeTrue();

                controller.Should().BeOfType(typeof(ControllerPS3));
